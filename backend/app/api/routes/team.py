...
def update_team_member_role(
    ...
    log_action(
        db, "team_member.role_updated", business_id=business.id, actor_user_id=current_user.id,
        target_type="team_member", target_id=str(member.id),
        details={"email": member.invited_email, "from_role": previous_role, "to_role": updated.role},
        ip_address=client_ip(request),
    )
...
def remove_team_member(
    ...
    member = get_owned_team_member(member_id, business, db)
    removed_email, removed_role = member.invited_email, member.role
    remove_member(db, business, member)
    log_action(
        db, "team_member.removed", business_id=business.id, actor_user_id=current_user.id,
        target_type="team_member", target_id=str(member_id),
